...
### 3.4 Beads: Memory & Coordination System (External Integration)

git-copilot 采用**双系统架构**来实现记忆与任务协调:

- **External Beads (steveyegge/beads)**: 基于 Dolt 的任务跟踪与协调系统
- **Custom Memory**: 轻量级代码审查发现存储 (SQLite + 关键词检索)

两者协同工作,为多 Agent 并行审查提供任务编排与上下文记忆能力。

#### 3.4.1 External Beads: 任务协调层 (steveyegge/beads)

**简介**: Beads 是一个分布式的、基于 Dolt SQL 数据库的 Issue Tracker,专为 AI Agents 设计。它提供:
- 任务创建、认领、状态管理
- 依赖图与阻塞检测 (`bd ready`)
- 哈希 ID (bd-a1b2) 避免多 Agent 冲突
- 任务记忆压缩 (compaction)
- 支持 Git 与 Git-Free 模式

**安装**: 用户需提前安装 (git-copilot init 会检测):
```bash
# npm 全局安装
...
**架构角色**:
- **Epic Task**: 每次代码审查创建一个顶层任务: `bd create "Code review: PR#123"`
- **Sub-tasks**: 每个 Agent 创建一个子任务 (Security, Performance, ...)
- **Task Dependencies**: 可设置依赖关系,自动阻塞检测
- **Agent Claiming**: Agents 通过 `bd update <id> --claim` 原子认领任务
- **Task Closure**: 完成后通过 `bd close <id> "reason"` 关闭

**数据关联**: Beads Task ID 与 Custom Memory Finding 通过 `relatedTaskId` 字段双向链接。

#### 3.4.2 Custom Memory: 审查发现存储

由于 steveyegge/beads 专注于任务协调而非语义检索,我们保留一个轻量级的 **Custom Memory** 系统来存储实际的审查发现:
...
interface FindingBead {
  ...
  content: string;               // 问题描述 + 代码片段 (max 2000 chars)
  filePath?: string;             // 关联文件路径
  lineRange?: { start: number; end: number };
  priority: number;              // 1-10, 严重程度
  agentSource: string;           // 来源 Agent 名称
  createdAt: Date;
  tags: string[];                // 关键词: ['xss', 'sql-injection', 'n+1', ...]
  relatedTaskId?: string;        // 关联的 beads task ID (双向链接)
}
```

**存储**: SQLite 表 `findings` (better-sqlite3)
- 索引: `tags`, `type`, `priority`, `relatedTaskId`, `createdAt`
- 位置: `~/.git-copilot/data/findings.db`

**检索 API**:
- `storeFinding(finding): Promise<string>` - 存储发现,返回 bead ID
- `getFinding(id): Promise<FindingBead | null>`
- `searchFindings(query: string, limit?: number): Promise<FindingBead[]>` - 基于 tags/content 的 LIKE 关键词搜索
- `getFindingsByTask(taskId: string): Promise<FindingBead[]>` - 获取某任务关联的所有发现
- `clearFindings(filter?: {...}): Promise<void>` - 清理旧数据 (90天 TTL)

**限制**:
- 不使用向量 embedding (v1.0 MVP 简化)
- 仅支持关键词搜索 (tags/content LIKE %query%)
- 语义检索功能延后至 v1.1
...
```
1. Workflow Init
   └─ beadsClient.createTask("Code review: PR#123") → epicId
   └─ shared_store.beadsEpicId = epicId

2. Agent Task Creation (per agent)
   ├─ beadsClient.createSubTask(epicId, "Security scan") → taskId
   ├─ beadsClient.claimTask(taskId)  // 原子认领
   └─ Agent stores findings with relatedTaskId = taskId

3. Context Retrieval (per agent)
   └─ customMemory.searchFindings(query) → previous relevant findings
      // 查询 tags/content 匹配,按 priority 排序
      // 不进行 embedding 相似度计算 (v1.0 简化)

4. Cross-Agent Sharing
   ├─ SecurityAgent finding → stored with taskId = bd-a1b2
   ├─ PerformanceAgent → memory.getFindingsByTask(bd-a1b2) 可以看到 Security 的发现
   └─ 通过 relatedTaskId 实现跨 Agent 上下文传递

5. Workflow Completion
   ├─ 每个 Agent 关闭自己的 task: beadsClient.closeTask(taskId, summary)
   ├─ Aggregator 生成报告
   └─ 清理 shared_store
```

**阻塞检测**: 如果任务 A 依赖于任务 B,则 `bd ready` 不会返回 A,直到 B 完成。git-copilot 可利用此特性协调串行依赖的 Agent。

#### 3.4.4 Configuration

`~/.git-copilot/config.yaml` 中 beads 相关配置:

```yaml
beads:
  external:
    enabled: true              # 是否使用 steveyegge/beads (false 则仅用 Custom Memory)
    autoInstall: false        # 自动安装 (init 时检测并提示)
    cliPath: "bd"             # bd 命令路径 (which bd)
    dataDir: ".beads"         # beads 存储目录 (项目根目录下)

  custom:
    enabled: true
    maxFindingsPerTask: 100   # 每个 task 最多存储的 finding 数量 (防止爆炸)
    retentionDays: 90         # 清理超过 90 天的 findings
    maxContextTokens: 4096    # context 注入时的 Token 上限
```

**环境变量**:
- `BEADS_DIR`: beads 数据目录 (覆盖 dataDir)
- `GIT_COPILOT_BEADS_AUTO_INSTALL`: 是否自动安装 (true/false)

#### 3.4.5 CLI 命令集成

git-copilot 提供的 Beads 管理命令 (可选):

```bash
# 检测 beads 安装状态
git-copilot beads check

# 初始化 beads (如果未 init)
git-copilot beads init

# 查看当前审查的 task 列表
git-copilot beads tasks --ready

# 查看 task 详情
git-copilot beads show <task-id>
```
