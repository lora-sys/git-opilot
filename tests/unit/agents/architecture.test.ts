...
const mockLLM = {
  ...
  countTokens: vi.fn().mockResolvedValue(120),
}
...
describe('ArchitectureAgent', () => {
  ...
  describe('analyze method', () => {
    it('should return proper AgentResult with findings', async () => {
      const files = [{ path: 'app.ts', content: 'code' }]
      const mockFindings = [{ type: 'solid-violation', severity: 'high', message: 'Single Responsibility violated' }]
  ...
  describe('Beads integration', () => {
    it('should create and claim Beads sub-task', async () => {
      ...
      expect(beads.createSubTask).toHaveBeenCalledWith('epic-123', expect.stringContaining('architecture'))
