...
import type { FileContent, AgentResult, AgentFinding, AgentDependencies, SharedStore, Logger } from './types.js'
...
export abstract class BaseAgent {
  ...
  protected memory: MemoryManager | undefined
  protected beadsClient: BeadsExternalClient | undefined
  protected logger: Logger

  private taskId?: string // Beads task ID for this agent run

  constructor(name: string, type: string, llm: LLMAdapter, config: any, deps: AgentDependencies) {
    this.name = name
    this.type = type
    this.llm = llm
    this.config = config
    ...
    this.logger = deps.logger! // logger is required in deps
  }

  abstract analyze(files: FileContent[], context?: any): Promise<AgentResult>

  protected abstract buildPrompt(files: FileContent[], context: any): Promise<import('../llm/adapter.js').ChatMessage[]>
